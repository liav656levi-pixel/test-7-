    ...
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-sans" dir="rtl">
          ...
          <div className="fixed inset-0 z-[150] flex justify-start">
            ...
            >
              ...
              <nav className="flex-1 p-8 space-y-6">
                {[{ id: 'about', label: 'הסיפור שלנו' }, { id: 'menu', label: 'התפריט שלנו' }, { id: 'contact', label: 'צור קשר' }].map((item) => (
                  <button key={item.id} onClick={(e) => smoothScroll(e, item.id)} className="block w-full text-right text-xl font-bold hover:text-primary transition-colors">
                    {item.label}
                  </button>
                ))}
              </nav>
              <div className="p-8 border-t border-primary/10 space-y-4">
                <div className="flex gap-4">
                  <a href="https://instagram.com/liavbakery" target="_blank" rel="noopener noreferrer" className="text-primary hover:opacity-70 transition-opacity"><Instagram className="w-6 h-6" /></a>
                  <a href="tel:0555567714" className="text-primary hover:opacity-70 transition-opacity"><Phone className="w-6 h-6" /></a>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
                {cartItems.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-6 text-center">
                    <div className="w-24 h-24 rounded-full bg-primary/5 flex items-center justify-center"><ShoppingCart className="w-10 h-10 opacity-20 text-primary" /></div>
                    <div><p className="text-xl font-bold">הסל שלך ריק כרגע</p><p className="text-sm mt-2 opacity-60">הלחמים הטריים שלנו מחכים לך...</p></div>
                      ...
                      <motion.div layout key={item.cartId} className="group flex gap-5 bg-white dark:bg-white/5 p-5 rounded-3xl border border-primary/5 shadow-sm hover:shadow-md transition-all">
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            ...
                            {item.addOns.length > 0 && <p className="text-[10px] text-primary font-bold uppercase tracking-wider mt-2 opacity-70">{item.addOns.map(id => ADD_ONS.find(a => a.id === id)?.name).join(' • ')}</p>}
                          </div>
                          <div className="flex gap-4 mt-4">
                            <button onClick={() => openEditCartItem(item)} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors flex items-center gap-1">עריכה</button>
                            <button onClick={() => removeFromCart(item.cartId)} className="text-[10px] font-black uppercase tracking-widest text-red-400 hover:text-red-500 transition-colors flex items-center gap-1">הסרה</button>
                          </div>
                        </div>
                      </motion.div>
                    ))}
              ...
              {cartItems.length > 0 && (
                <div className="p-8 border-t border-primary/10 bg-white dark:bg-black/20">
                  <div className="flex items-end justify-between mb-6">
                    <div className="flex flex-col"><span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">סה"כ לתשלום</span><span className="text-4xl font-black text-primary">₪{getCartTotal()}</span></div>
                  </div>
                  <div className="flex items-center gap-2 mb-8 p-3 bg-primary/5 rounded-2xl border border-primary/10">
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center"><ReceiptText className="w-4 h-4 text-primary" /></div>
                    <div><p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">תשלום ב-Bit או Paybox</p><p className="text-xs font-black text-primary">055-5567714</p></div>
                  </div>
          ...
          <div className="fixed inset-0 z-[130] flex items-end sm:items-center justify-center p-0 sm:p-6">
            ...
            >
                ...
                <div className="absolute bottom-6 right-8 left-8 text-white">
                  ...
                  {customizingProduct.category === "ללא גלוטן" && (
                    <div className="mt-4 p-3 bg-red-500/20 backdrop-blur-md border border-red-500/30 rounded-xl">
                      <p className="text-[10px] text-red-200 font-bold leading-tight">שימו לב: אנחנו עושים כל מה שאפשר על מנת להפריד, אך המטבח אינו סטרילי מגלוטן.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-8 pt-2 overflow-y-auto no-scrollbar">
                <div className="space-y-4 mb-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {ADD_ONS.map((addon) => (
                      <button key={addon.id} onClick={() => toggleAddOn(addon.id)} className={`group flex items-center justify-between p-5 rounded-[2rem] border-2 transition-all ${selectedAddOns.includes(addon.id) ? "border-primary bg-primary/5 shadow-inner" : "border-slate-200 dark:border-slate-800 hover:border-primary/30 bg-white dark:bg-white/5"}`}>
                        ...
                        <span className={`text-xs font-black ${selectedAddOns.includes(addon.id) ? "text-primary" : "text-slate-400"}`}>
                          {addon.id === 'plain' ? "₪0" : (selectedAddOns.some(id => id !== 'plain') && selectedAddOns.includes(addon.id) ? `₪${(5 / selectedAddOns.filter(id => id !== 'plain').length).toFixed(1)}` : "₪5")}
                        </span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="p-8 bg-white dark:bg-black/20 border-t border-primary/10 flex items-center justify-between gap-8">
                <div className="flex flex-col"><span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">סה"כ</span><span className="text-3xl font-black text-primary">₪{calculateTotalPrice()}</span></div>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 glass px-6 py-5 flex items-center justify-between border-b border-primary/10 transition-transform duration-500 ease-in-out ${showHeader ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="flex items-center">
          <button onClick={() => setShowNav(true)} className="p-3 bg-primary/5 hover:bg-primary/10 text-primary rounded-2xl transition-all border border-primary/10" aria-label="תפריט"><Menu className="w-6 h-6" /></button>
        </div>
      ...
      <div className="fixed bottom-8 left-8 z-[100]">
        <button onClick={() => setShowCart(true)} className="relative p-5 bg-primary text-white rounded-full shadow-2xl shadow-primary/40 hover:scale-110 active:scale-95 transition-all group">
          ...
          <AnimatePresence>{getCartItemsCount() > 0 && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full bg-white text-primary text-xs font-black shadow-xl border-2 border-primary">{getCartItemsCount()}</motion.span>}</AnimatePresence>
          <span className="absolute right-full mr-4 top-1/2 -translate-y-1/2 bg-white dark:bg-slate-800 text-primary dark:text-white px-4 py-2 rounded-xl text-xs font-black shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-primary/10">הסל שלי: ₪{getCartTotal()}</span>
        </button>
      </div>
      ...
      <main className="flex-1 pb-12 pt-24">
        ...
        <section id="about" className="px-6 py-16 bg-primary/5 my-12 rounded-[3rem] mx-6">
          <div className="max-w-3xl mx-auto text-center">
            <h3 className="text-3xl font-black mb-8">הסיפור שלנו</h3>
            <div className="space-y-6 text-slate-600 dark:text-slate-400 text-lg leading-relaxed font-medium">
              <p>הכל התחיל מחשק אמיתי ללחם טוב כזה איכותי בלי פשרות. הרצון לייצר לחם שעונה על הצרכים של כולם - תערובת קמחים ללא גלוטן, כוסמין מלא, חיטה מלאה.</p>
              <p>אצלנו המחמצת עוברת תסיסה ארוכה של 24 שעות להעמקת טעמים ולהגיע למרקם נכון יותר. כל לחם שתזמינו נעשה במיוחד עבורכם מקמחי מקור איכותיים ומחמצת טבעית בלבד בלי שמרים תעשייתיים ובלי שום קיצורי דרך.</p>
              <p className="text-primary font-bold">בדיוק כמו שלחם צריך להיות :)</p>
            </div>
            <div className="mt-10 flex justify-center gap-4">
              <div className="h-[1px] w-12 bg-primary/20 self-center" /><span className="font-bold text-primary tracking-widest uppercase">Sabrosa</span><div className="h-[1px] w-12 bg-primary/20 self-center" />
            </div>
          </div>
        </section>
        ...
        <div id="menu" className="px-6 mb-12">
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <div className="mt-4 flex justify-center gap-4">
              <div className="h-[1px] w-12 bg-primary/20 self-center" /><div className="w-2 h-2 rounded-full bg-primary/20 self-center" /><div className="h-[1px] w-12 bg-primary/20 self-center" />
            </div>
          </div>
              ...
              <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={product.id} className="group flex flex-col bg-white dark:bg-white/5 rounded-[2.5rem] overflow-hidden border border-primary/5 hover:shadow-2xl transition-all duration-500">
                ...
                <div className="p-8 flex-1 flex flex-col">
                  <div className="flex-1">
                    ...
                    {product.id === 1 && <div className="flex items-center gap-1 mb-3 text-primary"><Star className="w-3 h-3 fill-current" /><span className="text-[10px] font-black uppercase tracking-widest">המומלץ שלנו</span></div>}
                    <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed line-clamp-3 min-h-[4.5rem]">{product.description}</p>
                    {product.category === "ללא גלוטן" && <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl"><p className="text-[10px] text-red-500 font-bold leading-tight">שימו לב: אנחנו עושים כל מה שאפשר על מנת להפריד, אך המטבח אינו סטרילי מגלוטן.</p></div>}
                  </div>
                  <div className="flex gap-3 mt-auto pt-8"><button onClick={() => openCustomization(product)} className="flex-1 bg-primary text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-primary/10">הוסף לסל</button></div>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4 group">
                    <div className="w-12 h-12 rounded-2xl bg-primary/5 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all"><Phone className="w-5 h-5" /></div>
                  ...
                  <a href="https://instagram.com/liavbakery" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 group hover:opacity-80 transition-opacity">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-primary/5 flex items-center justify-center"><ReceiptText className="w-5 h-5" /></div>
                      <div><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">אפשרויות תשלום</p><p className="font-bold">Bit / Paybox</p></div>
                    </div>
                  </div>
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-3">
                      <a href="https://waze.com/ul?ll=31.8219,34.8291&navigate=yes" target="_blank" rel="noopener noreferrer" className="text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary px-4 py-2 rounded-xl hover:bg-primary hover:text-white transition-all">Waze</a>
                      <a href="https://www.google.com/maps/search/?api=1&query=מושב+קדרון" target="_blank" rel="noopener noreferrer" className="text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary px-4 py-2 rounded-xl hover:bg-primary hover:text-white transition-all">Google Maps</a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-white dark:bg-white/5 p-8 rounded-[2.5rem] border border-primary/5 shadow-xl">
                ...
                <div className="mt-8 p-4 bg-primary/5 rounded-2xl text-center"><p className="text-xs font-bold text-primary">האפייה מתבצעת במיוחד עבורכם לפי הזמנה</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 pt-12 border-t border-primary/5">
            ...
            <div className="text-center">
              ...
              <div className="text-sm font-bold space-y-1"><p>חמישי: 17:00 - 21:00</p><p>שישי: 08:00 - 12:00</p></div>
            </div>
            <div className="text-center sm:text-left">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-4">חברתי</h4>
              <div className="flex justify-center sm:justify-start gap-4">
                <a href="https://instagram.com/liavbakery" target="_blank" rel="noopener noreferrer" className="p-2 bg-primary/5 rounded-xl hover:bg-primary hover:text-white transition-all"><Instagram className="w-5 h-5" /></a>
                <a href="tel:0555567714" className="p-2 bg-primary/5 rounded-xl hover:bg-primary hover:text-white transition-all"><Phone className="w-5 h-5" /></a>
              </div>
            </div>
          </div>
